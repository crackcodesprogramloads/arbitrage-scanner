...
export default function Searchbar() {
  ...
  const [coinData, setCoinData] = useState<{ name: string; ticker: string }[]>([]);
  ...
  const handleCoinRetrieval = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    for (const key in filteredCoins) {
      if (filteredCoins.hasOwnProperty(key)) {
        const item = filteredCoins[key] as { name: string; ticker: string };
        filteredCoinsArray.push({ name: item.name, ticker: item.ticker });
      }
    }
...
// const handleCoinIsStarred = async (event: any, coin: { name: string; ticker: string; inWatchlist: boolean }) => {
...
//   console.log("handleCoinIsStarred has been clicked");
//   await addToWatchlist("c2fd2d03-5233-4b25-8b02-0944a7d7b33d", coin.name, coin.ticker);
//   console.log("addToWatchlist has finished");
// };
